 ...
 *
 * isProQuotaExceededError was removed in aioncli-core v0.18.4; this is the local replacement.
 */
function isProQuotaExceededError(error: unknown): boolean {
  if (!error || typeof error !== 'object') return false;

  // Check whether the error contains Pro-model quota-related information
 ...
 *
 * isGenericQuotaExceededError was removed in aioncli-core v0.18.4; this is the local replacement.
 */
function isGenericQuotaExceededError(error: unknown): boolean {
  if (!error || typeof error !== 'object') return false;

  // Check whether the error contains quota-related information
  const errorStr = JSON.stringify(error).toLowerCase();
  return (
    errorStr.includes('quota') &&
