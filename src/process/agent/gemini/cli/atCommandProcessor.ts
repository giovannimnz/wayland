...
interface HandleAtCommandParams {
  ...
  signal: AbortSignal;
  /**
   * Lazy loading mode: don't read file content immediately, only pass file path hints to the agent.
   * Let the agent decide when to use the read_file tool to read files.
   */
...
export async function handleAtCommand({
  ...
  // Lazy loading mode: don't read file content, only pass file path hints to the agent.
  if (lazyFileLoading) {
